...
//		}
	 ...
	 *
	 * @throws LibrecException
	 *             if error occurs during training model
	 */
...
//			recommendedList.topNRankItemsByUser(userIdx, topN);
	 ...
	 * @throws LibrecException
	 *             if error occurs during predicting
	 */
				...
				while (recommendedEntryIter.hasNext()) {
					...
					if (userItemRatingEntry != null) {
						...
						if (StringUtils.isNotBlank(userId) && StringUtils.isNotBlank(itemId)) {
							userItemList
									.add(new GenericRecommendedItem(userId, itemId, userItemRatingEntry.getValue()));
						}
					}
				}
				return userItemList;
